...
public class EnderitedimensionTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(MoreVanillaModBlocks.ENDERITEDIMENSION_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("more_vanilla:enderitedimension_portal"), poiType);
	...
	public EnderitedimensionTeleporter(ServerLevel level) {
		this.level = level;
	}
			...
			for (int i3 = -1; i3 < 2; i3++) {
				for (int j3 = 0; j3 < 2; j3++) {
					for (int k3 = -1; k3 < 3; k3++) {
						BlockState blockstate1 = k3 < 0 ? MoreVanillaModBlocks.ENDSTONEMODIFIED.get().defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		for (int l1 = -1; l1 < 3; l1++) {
			for (int j2 = -1; j2 < 4; j2++) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, MoreVanillaModBlocks.ENDSTONEMODIFIED.get().defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = MoreVanillaModBlocks.ENDERITEDIMENSION_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
