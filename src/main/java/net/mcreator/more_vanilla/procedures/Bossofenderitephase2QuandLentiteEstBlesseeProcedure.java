...
public class Bossofenderitephase2QuandLentiteEstBlesseeProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (a == 4) {
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = MoreVanillaModEntities.ENDSILVER.get().spawn(_level, BlockPos.containing(x + 1, y + 1, z - 1), MobSpawnType.MOB_SUMMONED);
				if (entityToSpawn != null) {
					entityToSpawn.setYRot(world.getRandom().nextFloat() * 360F);
				}
			}
